...
# then restart DBX. Old io.dbx.kafka installed versions are cleaned up unless
# --keep-old is passed (rollback safety).
#
# Usage:
...
#   scripts/install.sh --keep-old         # keep older io.dbx.kafka versions for rollback
#
# Environment:
...
resolve_host_worktree() {
  ...
  host_worktree_is_valid "$sibling_root/dbx" && candidate_list+=("$sibling_root/dbx")
...
if [ "$REINSTALL" = 1 ]; then
  VERSION_DIR="$PLUGIN_STORE/io.dbx.kafka/versions/$VERSION"
  if [ -d "$VERSION_DIR" ]; then
    echo "==> dev reinstall: removing installed v$VERSION"
    rm -rf "$VERSION_DIR"
    python3 - "$PLUGIN_STORE/io.dbx.kafka/activations" "$VERSION" <<'PY'
...
if [ "$KEEP_OLD" = 0 ]; then
  echo "==> cleaning old io.dbx.kafka versions (keeping v$VERSION)"
...
versions_dir = plugin_store / "io.dbx.kafka" / "versions"
activations_dir = plugin_store / "io.dbx.kafka" / "activations"
...
PY
  ...
  echo "==> keeping old io.dbx.kafka versions (--keep-old)"
fi
